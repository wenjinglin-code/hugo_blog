+++
title = "异常,中断"
date = 2026-03-26T19:22:22.782092+08:00
tags = ["riscv", "linux"]
...
- s-mode 只包含 s，u 的中断，因此不能处理 m 中断
...
- 每个特权都有一套中断控制寄存器控制每个中断：
    - `xmie`：中断使能 csr，如下为 `mie` 的格式，每个对应的 bit 对应一个中断的使能位
        
        ![image.png](image.png)
        
    - `xmip`：中断 pending csr，如下为 mip 的格式，如果实现了 software interrupts，可以通过写 `xSIP` ，软件触发一个中断
        
        ![image.png](image%201.png)
        
- 每个特权可以通过 `xstatus.xIE` 控制某个特权的总中断使能，格式如下：
    
    ![image.png](image%202.png)
    
    在触发中断时，硬件会自动将 `xstatus.xIE` 拷贝到 `xstatus.xPIE`，并 clear 掉，以防止中断嵌套；
    
    在中断返回时（执行 `xret`），硬件会自动将 `xstatus.xPIE` 拷贝到 `xstatus.xIE` ，以恢复中断使能；
    
- 一个中断要触发，需要满足表达式：`xmie.xx && xstatus.xIE`
...
- 被配置为 ebreak 触发，进入 debug mode（`dcsr.ebreakx == 1`，详细 [Debug Mode](../riscv%20%E5%90%AF%E5%8A%A8%20dd1524dd01b546478d642fd86909e028.md) ）：
...
|- .Lsave_context // 将数据压入内核栈
	 ...
	 |- bge s4, zero, 1f // 根据 CSR_CAUSE 判断是 异常还是中断，中断 最高位为 1，体现为负数，大于就会不成立
...
csr_xtopi // 中断源寄存器，保存主中断号，例如，保存 s-externel/s-timer interrupt num
csr_xtopei // 扩展中断源寄存器，imsic 实现，保存配置 imsic 中断时配置的 imsic hwirq，例如，target[i].EIID
```
...
不同的特权等级有对应的 `interrupt domain`，每个特权等级连接 cpu 各个特权等级的 `external interrupt`，或者发送到 imsic 对应特权等级的 `interrupt file`；

如果使用 msi 模式，`tartget[x]` 决定了硬件中断 x，往 imsic 发的 `msi msg`；通过 `xmsiaddrcfg，xmsiaddrcfgh` 决定发送到哪个 `interrupt file`；
...
每个 hart 对应一组 `interrupt files`，其中包括 `一个 m-file，一个 s-file，hsxlen 个 g-files`；

所有的 interrupt files 都是映射到同块内存，每个 interupt file 占用 `4k`，各个 interupt file 连续排列，排完一个 hart 再排下一个 hart；

![Untitled](Untitled%2010.png)

### *iselect 间接访问 `g interrupt file` 寄存器
...
linux 中 msi address 计算方式：

```jsx
                     |      group_index_shift - 12      |     
  ...
  Group Index 对应图中的 hart idex high
  HART Index 对应图中的 hart iex low
  group_index_shift - 12 对应图中的 HHXS + 12，一般会从 dts 传递给 linux kernel
```
	 ...
	 |- imsic_early_dt_init // 注册 **ipi_mux_domain**  
		  |- rc = imsic_setup_state(fwnode)
			   |- rc = imsic_parse_fwnode(fwnode, global, &nr_parent_irqs, &nr_mmios); // 初始化 **imsic->global**
				 ...
				 |- mmios_va[i] = ioremap(mmios[i].start, resource_size(&mmios[i])); // remap s-file memory, 以便后面发ipi 时使用，映射前会 mmios[i].start 是否符合规范
				 |- rc = imsic_local_init(); // 初始化 per_cpu_ptr(imsic->lpriv, cpu)，主要用来记录各个cpu imsic 各个中断（**lpriv->vectors[i]**）的状态
				 ...
				 |- rc = imsic_matrix_init(); // 初始化 imsic->matrix，用来申请 imsic 中断时，获取唯一中断号
			|- rc = imsic_early_probe(fwnode); 
				 ...
				 |- rc = imsic_ipi_domain_init();
						|- virq = ipi_mux_create(IMSIC_NR_IPI, imsic_ipi_send);  // 注册 **ipi_mux_domain** 用于管理 ipi，使用 IMSIC_IPI_ID 作为 imsic 的 hwirq
				 ...
				 |- irq_set_chained_handler(imsic_parent_irq, **imsic_handle_irq**); // 改变 desc-> handle, 使 **intc_domain** 中 RV_IRQ_EXT 中断，走 imsic 的中断处理, **riscv_intc_aia_irq -> imsic_handle_irq** 
...
[RISC-V SBI IPI 实现说明](.//RISC-V%20SBI%20IPI%20%E5%AE%9E%E7%8E%B0%E8%AF%B4%E6%98%8E.md)